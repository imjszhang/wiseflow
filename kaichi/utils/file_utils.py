...
def sanitize_filename(filename):
    # 定义一个正则表达式模式，该模式匹配所有在Windows和Unix文件系统中不允许的字符
    pattern = r'[\\/:*?"<>|]'
    sanitized = re.sub(pattern, '_', filename)  # 使用下划线替换所有匹配的字符
    return sanitized
...
def load_pickle(*fpaths):
    with open(f_join(*fpaths), "rb", encoding="UTF-8") as fp:
        return pickle.load(fp)


def dump_pickle(data, *fpaths):
    with open(f_join(*fpaths), "wb", encoding="UTF-8") as fp:
        pickle.dump(data, fp)
